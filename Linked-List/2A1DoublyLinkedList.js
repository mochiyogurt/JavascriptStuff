function DoublyLinkedList1() {
        ...
        this.append = function(element){
            ...
            if (head === null){ //first node on list, 1 item so head and tail is the same
                ...
                tail.next = node;    //Make the first item bagel's next as the new node cream cheese
                node.prev = tail;    //Make the new node Cream Cheese prev as the original tail bagel
                tail = node;         //Now change the tail to the new node Cream Cheese
            }
        ...
        this.insert = function(position, element){
            ...
            if (position >= 0 && position <= length){
                ...
                {
                    //1,2,4,5
                    //inserting in the middle  3 between 1 and 2
                    while (index++ < position){
                        previous = current;      //previous is equal to current aka head  previous=1
                        current = current.next;  //current is set to current aka head's next current=2
                    }
                    node.next = current;         //the new node's next is set as current   new node 3's next = 2
                    previous.next = node;        //the previous next is now set as the new node,  previous=1 1's next = 3
    
                    current.prev = node; //NEW    //current's prev = 2's previous is 3
                    node.prev = previous; //NEW   //node's prev = 3's previous is 1 aka previous = current = head from above
                }
    ...
    let list = new DoublyLinkedList1();
    list.append(1);
    list.append(2);
    list.append(3);
    list.append(5);
